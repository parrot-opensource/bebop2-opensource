...
#ifdef LIBMTP_SUPPORT_VENDOR_IRIVER
...
ptp_usb_event (PTPParams* params, PTPContainer* event, int wait)
{
	...
	if (ret == PTP_ERROR_TIMEOUT) {
		libusb_glue_debug (params,
			"PTP: reading event TIMEOUT occurred");
		return ret;
	}
...
uint16_t
ptp_usb_event_wait_timeout (PTPParams* params, PTPContainer* event,
	int timeout) {

	/* Work around the two special values of timeout */

	if (timeout == PTP_EVENT_CHECK_FAST)
		timeout = 2;  /* minimum possible timeout */
	if (timeout == PTP_EVENT_CHECK)
		timeout = 0;  /* nothing to do */
