...
{
  LIBMTP_Dump_Device_Info_Ex(device, printf);
}

void LIBMTP_Dump_Device_Info_Ex(LIBMTP_mtpdevice_t* device,
    printf_like_callback _printf)
{
  int i;
  PTPParams *params = (PTPParams *) device->params;
  PTP_USB *ptp_usb = (PTP_USB*) device->usbinfo;
  ...
  _printf("USB low-level info:\n");
  dump_usbinfo(ptp_usb);
  /* Print out some verbose information */
  _printf("Device info:\n");
  _printf("   Manufacturer: %s\n", params->deviceinfo.Manufacturer);
  _printf("   Model: %s\n", params->deviceinfo.Model);
  _printf("   Device version: %s\n", params->deviceinfo.DeviceVersion);
  _printf("   Serial number: %s\n", params->deviceinfo.SerialNumber);
  _printf("   Vendor extension ID: 0x%08x\n",
	 params->deviceinfo.VendorExtensionID);
  _printf("   Vendor extension description: %s\n",
	 params->deviceinfo.VendorExtensionDesc);
  _printf("   Detected object size: %d bits\n",
  ...
  _printf("Device Properties Supported:\n");
  for (i=0;i<params->deviceinfo.DevicePropertiesSupported_len;i++) {
    ...
    if (propdesc != NULL) {
      ...
      _printf("   0x%04x: Unknown property\n", prop);
    }
  }

  if (ptp_operation_issupported(params,PTP_OC_MTP_GetObjectPropsSupported)) {
    _printf("Playable File (Object) Types and Object Properties Supported:\n");
	...
	for (j=0;j<propcnt;j++) {
	  ...
	  if (opd.DataType == PTP_DTC_STR) {
	    _printf(" STRING data type");
	    switch (opd.FormFlag) {
	    case PTP_OPFF_DateTime:
	      _printf(" DATETIME FORM");
	      break;
	    case PTP_OPFF_RegularExpression:
	      _printf(" REGULAR EXPRESSION FORM");
	      break;
	    case PTP_OPFF_LongString:
	      _printf(" LONG STRING FORM");
	      break;
	    default:
	      break;
	    }
	    ...
	    case PTP_DTC_UNDEF:
	      _printf(" UNDEFINED data type");
	      break;
	    case PTP_DTC_INT8:
	      _printf(" INT8 data type");
	      switch (opd.FormFlag) {
	      case PTP_OPFF_Range:
		_printf(" range: MIN %d, MAX %d, STEP %d",
		...
		for(k=0;k<opd.FORM.Enum.NumberOfValues;k++) {
		  _printf("%d, ", opd.FORM.Enum.SupportedValue[k].i8);
		}
		break;
		...
		break;
	      default:
		_printf(" ANY 8BIT VALUE form");
		break;
	      }
	      break;

	    case PTP_DTC_UINT8:
	      _printf(" UINT8 data type");
	      switch (opd.FormFlag) {
	      case PTP_OPFF_Range:
		_printf(" range: MIN %d, MAX %d, STEP %d",
		...
		_printf(" enumeration: ");
		for(k=0;k<opd.FORM.Enum.NumberOfValues;k++) {
		  _printf("%d, ", opd.FORM.Enum.SupportedValue[k].u8);
		}
		break;
	      case PTP_OPFF_ByteArray:
		_printf(" byte array: ");
		...
		_printf(" enumeration: ");
		for(k=0;k<opd.FORM.Enum.NumberOfValues;k++) {
		  _printf("%d, ", opd.FORM.Enum.SupportedValue[k].i16);
		}
		break;
	      default:
		_printf(" ANY 16BIT VALUE form");
		break;
	      }
	      break;

	    case PTP_DTC_UINT16:
	      _printf(" UINT16 data type");
	      switch (opd.FormFlag) {
	      case PTP_OPFF_Range:
		_printf(" range: MIN %d, MAX %d, STEP %d",
		...
		_printf(" enumeration: ");
		for(k=0;k<opd.FORM.Enum.NumberOfValues;k++) {
		  _printf("%d, ", opd.FORM.Enum.SupportedValue[k].u16);
		}
		break;
	      default:
		_printf(" ANY 16BIT VALUE form");
		...
		_printf(" enumeration: ");
		for(k=0;k<opd.FORM.Enum.NumberOfValues;k++) {
		  _printf("%d, ", opd.FORM.Enum.SupportedValue[k].i32);
		}
		break;
	      default:
		_printf(" ANY 32BIT VALUE form");
		break;
	      }
	      break;

	    case PTP_DTC_UINT32:
	      _printf(" UINT32 data type");
	      switch (opd.FormFlag) {
	      case PTP_OPFF_Range:
		_printf(" range: MIN %d, MAX %d, STEP %d",
		...
		if (params->deviceinfo.ImageFormats[i] == PTP_OPC_VideoFourCCCodec) {
		  _printf(" enumeration of u32 casted FOURCC: ");
		  for (k=0;k<opd.FORM.Enum.NumberOfValues;k++) {
		  ...
		  _printf(" enumeration: ");
		  for(k=0;k<opd.FORM.Enum.NumberOfValues;k++) {
		    _printf("%d, ", opd.FORM.Enum.SupportedValue[k].u32);
		  }
		}
		break;
	      default:
	    ...
	    case PTP_DTC_INT64:
	      _printf(" INT64 data type");
	      break;

	    case PTP_DTC_UINT64:
	      _printf(" UINT64 data type");
	      break;

	    case PTP_DTC_INT128:
	      _printf(" INT128 data type");
	      break;

	    case PTP_DTC_UINT128:
	      _printf(" UINT128 data type");
	      break;

	    default:
	      _printf(" UNKNOWN data type");
	      break;
	    }
	  }
	  if (opd.GetSet) {
  ...
  if(storage != NULL &&
    ...
    _printf("Storage Devices:\n");
    while(storage != NULL) {
      _printf("   StorageID: 0x%08x\n",storage->id);
      _printf("      StorageType: 0x%04x ",storage->StorageType);
      switch (storage->StorageType) {
      case PTP_ST_Undefined:
	_printf("(undefined)\n");
	break;
      case PTP_ST_FixedROM:
	_printf("fixed ROM storage\n");
	break;
      case PTP_ST_RemovableROM:
	_printf("removable ROM storage\n");
	break;
      case PTP_ST_FixedRAM:
	_printf("fixed RAM storage\n");
	break;
      case PTP_ST_RemovableRAM:
	_printf("removable RAM storage\n");
	break;
      default:
	_printf("UNKNOWN storage\n");
	break;
      }
      _printf("      FilesystemType: 0x%04x ",storage->FilesystemType);
      switch(storage->FilesystemType) {
      case PTP_FST_Undefined:
	_printf("(undefined)\n");
	break;
      case PTP_FST_GenericFlat:
	_printf("generic flat filesystem\n");
	break;
      case PTP_FST_GenericHierarchical:
	_printf("generic hierarchical\n");
	break;
      case PTP_FST_DCF:
	_printf("DCF\n");
	break;
      default:
	_printf("UNKNONWN filesystem type\n");
	break;
      }
      _printf("      AccessCapability: 0x%04x ",storage->AccessCapability);
	...
	_printf("read only + object deletion\n");
	break;
      default:
	_printf("UNKNOWN access capability\n");
	break;
      }
      _printf("      MaxCapacity: %llu\n",
	     (long long unsigned int) storage->MaxCapacity);
      _printf("      FreeSpaceInBytes: %llu\n",
	     (long long unsigned int) storage->FreeSpaceInBytes);
      _printf("      FreeSpaceInObjects: %llu\n",
	     (long long unsigned int) storage->FreeSpaceInObjects);
      _printf("      StorageDescription: %s\n",storage->StorageDescription);
      _printf("      VolumeIdentifier: %s\n",storage->VolumeIdentifier);
      storage = storage->next;
    }
  }

  _printf("Special directories:\n");
  _printf("   Default music folder: 0x%08x\n",
	 device->default_music_folder);
  _printf("   Default playlist folder: 0x%08x\n",
	 device->default_playlist_folder);
  _printf("   Default picture folder: 0x%08x\n",
	 device->default_picture_folder);
  _printf("   Default video folder: 0x%08x\n",
	 device->default_video_folder);
  _printf("   Default organizer folder: 0x%08x\n",
	 device->default_organizer_folder);
  _printf("   Default zencast folder: 0x%08x\n",
	 device->default_zencast_folder);
  _printf("   Default album folder: 0x%08x\n",
	 device->default_album_folder);
  _printf("   Default text folder: 0x%08x\n",
...
/**
 * Helper function that takes one PTP object and creates a
 * LIBMTP_track_t metadata entry.
 */
static LIBMTP_track_t *obj2track(LIBMTP_mtpdevice_t *device, PTPObject *ob)
{
  PTP_USB   *ptp_usb = (PTP_USB *)(device ? device->usbinfo : NULL);
  ...
  // Ignore stuff we don't want to handle...
  mtptype = map_ptp_type_to_libmtp_type(ob->oi.ObjectFormat);
  if ((LIBMTP_FILETYPE_FOLDER != mtptype) && 
      !LIBMTP_FILETYPE_IS_TRACK(mtptype) &&
      !LIBMTP_FILETYPE_IS_IMAGE(mtptype) &&
  ...
  track->filename   = (ob->oi.Filename) ? strdup(ob->oi.Filename) : NULL;
 ...
 * This function retrieves the contents of a certain folder
 * with id parent on a certain storage on a certain device.
 * The result contains both media and folders.
 * This function works for devices opened without cache option
 *
 * NOTE: the request will always perform I/O with the device.
 * @param device a pointer to the MTP device to report info from.
 * @param storage a storage on the device to report info from. If
 ...
 */
static LIBMTP_track_t *LIBMTP_Get_Medialisting_Uncached_Device(
		LIBMTP_mtpdevice_t *device,
		uint32_t const storage,
		uint32_t const parent,
		LIBMTP_progressfunc_t const callback,
		void const * const data)
{
  PTPParams *params  = (PTPParams *)device->params;
  PTP_USB   *ptp_usb = (PTP_USB*)device->usbinfo;
  LIBMTP_track_t  *retlist = NULL;
  LIBMTP_track_t **tail    = &retlist;
  PTPObjectHandles currentHandles;
  uint16_t ret;
  int i = 0;
  ...
  ret = ptp_getobjecthandles(params,
			     (0 == storage) ? PTP_GOH_ALL_STORAGE : storage,
			     PTP_GOH_ALL_FORMATS,
			     (0 == parent)  ? PTP_GOH_ROOT_PARENT : parent,
			     &currentHandles);
  if (ret != PTP_RC_OK) {
    add_ptp_error_to_errorstack(device, ret,
	"LIBMTP_Get_Medialisting_Uncached_Device(): could not get object handles.");
    return NULL;
  }
  ...
  for (i = 0; i < currentHandles.n; i++) {
    ...
    if (callback != NULL)
      callback(i, currentHandles.n, data);

    ret = ptp_object_want(params, currentHandles.Handler[i],
			  PTPOBJECT_OBJECTINFO_LOADED|PTPOBJECT_MTPPROPLIST_LOADED, &ob);
    if (ret != PTP_RC_OK)
      continue;

    // Add track to the tail
    (*tail) = obj2track(device, ob);
    if (*tail)
      tail    = &((*tail)->next);
  } // Handle counting loop
 ...
 * with id parent on a certain storage on a certain device.
 * The result contains both media and folders.
 * This function works for devices opened with cache only
 *
 * NOTE: the request will always perform I/O with the device.
 * @param device a pointer to the MTP device to report info from.
 * @param storage a storage on the device to report info from. If
 ...
 * @param data     user data for callback
 */
static LIBMTP_track_t *LIBMTP_Get_Medialisting_Cached_Device(
		LIBMTP_mtpdevice_t *device,
		uint32_t const storage,
		uint32_t const parent,
		LIBMTP_progressfunc_t const callback,
		void const * const data)
{
  ...
  if (!device->cached) {
    LIBMTP_ERROR("tried to use %s on a uncached device!\n", __func__);
    return NULL;
  }
  ...
  for (i = 0; i < params->nrofobjects; i++) {
    ...
    // Ignore stuff that isn't into the storage device or the parent
    if ((parent != ob->oi.ParentObject) ||
	((0 != storage) && (storage != ob->oi.StorageID)))
      continue;
 ...
 * with id parent on a certain storage on a certain device.
 * The result contains both media and folders.
 * This function works for devices opened with cache or not
 *
 * NOTE: the request will always perform I/O with the device.
 * @param device a pointer to the MTP device to report info from.
 * @param storage a storage on the device to report info from. If
 ...
 * @param data     user data for callback
 */
LIBMTP_track_t *LIBMTP_Get_Medialisting_With_Callback(
			LIBMTP_mtpdevice_t *device,
			uint32_t const storage,
			uint32_t const parent,
			LIBMTP_progressfunc_t const callback,
...
{
  if (!device) {
    LIBMTP_ERROR("tried to use %s on no device!\n", __func__);
    return NULL;
  }
  if (device->cached)
    return LIBMTP_Get_Medialisting_Cached_Device(device, storage, parent, callback, data);
  else
    return LIBMTP_Get_Medialisting_Uncached_Device(device, storage, parent, callback, data);
}
...
LIBMTP_track_t *LIBMTP_Get_Trackmetadata(LIBMTP_mtpdevice_t *device, uint32_t const trackid)
{
  ...
  if (!LIBMTP_FILETYPE_IS_TRACK(mtptype) &&
      !LIBMTP_FILETYPE_IS_IMAGE(mtptype) &&
      (LIBMTP_FILETYPE_FOLDER != mtptype) &&
      /*
 ...
 * @param maxbytes Maximum of bytes to read
 * @param put_func the function to call when we have data.
 * @param priv the user-defined pointer that is passed to
 *             <code>put_func</code>.
 * @param callback a progress indicator function or NULL to ignore.
 ...
 *           failure.
 */
int LIBMTP_Get_Partial_File_To_Handler(
		LIBMTP_mtpdevice_t *device,
		uint32_t const id,
		uint64_t offset,
		uint32_t maxbytes,
		...
		void const * const data)
{
  ...
  ret = ptp_object_want (params, id, PTPOBJECT_OBJECTINFO_LOADED, &ob);
  if (ret != PTP_RC_OK) {
    add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL, "LIBMTP_Get_Partial_File_To_Handler(): Could not get object info.");
    return -1;
  }
  if (ob->oi.ObjectFormat == PTP_OFC_Association) {
    add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL, "LIBMTP_Get_Partial_File_To_Handler(): Bad object format.");
    return -1;
  }
  if ((0 >= maxbytes) || (offset >= ob->oi.ObjectCompressedSize)
      || ((offset + maxbytes) > ob->oi.ObjectCompressedSize)) {
    add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL, "LIBMTP_Get_Partial_File_To_Handler(): Bad offset.");
    return -1;
  }

  // Callbacks
  ptp_usb->callback_active = 1;
  ptp_usb->current_transfer_total = maxbytes+
    PTP_USB_BULK_HDR_LEN+sizeof(uint32_t); // Request length, one parameter
  ptp_usb->current_transfer_complete = 0;
  ptp_usb->current_transfer_callback = callback;
  ptp_usb->current_transfer_callback_data = data;
  ...
  if ((offset >> 32 == 0)
    ...
    ret = ptp_getpartialobject_to_handler(params, id, (uint32_t)offset, maxbytes, &handler);
  } else if (!ptp_operation_issupported(params, PTP_OC_ANDROID_GetPartialObject64)) {
      add_error_to_errorstack(device, LIBMTP_ERROR_GENERAL,
        "LIBMTP_Get_Partial_File_To_Handler: GetPartialObject (32/64) is not supported");
      return -1;
  } else {
    /* can fail with error PTP_ERROR_DATA_EXPECTED (tested only on sony xperia z1) */
    ret = ptp_android_getpartialobject64_to_handler(params, id, offset, maxbytes, &handler);
  }
