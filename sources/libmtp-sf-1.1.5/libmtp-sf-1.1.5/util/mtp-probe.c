...
static int check_sysfs(char *sysfspath)
{
  ...
  ret = regcomp(&r, "\\:[0-9]+\\.[0-9]+$", REG_EXTENDED | REG_NOSUB);
  if (ret)
    return -1;
...
int main (int argc, char **argv)
{
  ...
  if (argc < 4) {
    logger(LOG_INFO, "need device path, busnumber, device number as argument\n");
    printf("0");
    exit(0);
  }
  ...
  logger(LOG_INFO, "checking bus %d, device %d: \"%s\"\n", busno, devno, fname);
  ...
  if (ret) {
    logger(LOG_INFO, "bus: %d, device: %d was an MTP device\n", busno, devno);
    printf("1");
  } else {
    logger(LOG_INFO, "bus: %d, device: %d was not an MTP device\n", busno, devno);
    printf("0");
  }
