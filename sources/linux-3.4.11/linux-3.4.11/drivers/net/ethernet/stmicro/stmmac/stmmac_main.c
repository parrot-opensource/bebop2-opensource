...
static void stmmac_adjust_link(struct net_device *dev)
{
	...
	if ((priv->pcs != STMMAC_PCS_RGMII) && (priv->pcs != STMMAC_PCS_TBI) &
	    (priv->pcs != STMMAC_PCS_RTBI) && priv->dma_cap.eee)
		eee_possible = (phy_init_eee(priv->phydev, 1) == 0);
	...
	if ((priv->eee_enabled = eee_possible))
...
static int stmmac_open(struct net_device *dev)
{
	...
	if (priv->phydev) {
		...
		cancel_delayed_work_sync(&priv->phydev->state_queue);
		schedule_delayed_work(&priv->phydev->state_queue, 0);
	}
				     ...
				     void __iomem *addr)
{
	...
	if(plat_dat->mac_addr && is_valid_ether_addr(plat_dat->mac_addr))
		memcpy(ndev->dev_addr, plat_dat->mac_addr, ETH_ALEN);
	else
