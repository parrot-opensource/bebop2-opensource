...
int run_test(struct test *test)
{
	...
	for(c = 0; c < test->feed_entries; c++) {
		...
		if(!c) {
			...
			rd->last_collected_time.tv_nsec = st->last_collected_time.tv_nsec = st->last_updated.tv_nsec = 1000*test->feed[c].microseconds;
		}
	}
...
int unit_test(long delay, long shift)
{
	...
	for(c = 0; c < 20 ;c++) {
		...
		if(!c) {
			...
			st->last_collected_time.tv_nsec = 1000*shift;
		}
