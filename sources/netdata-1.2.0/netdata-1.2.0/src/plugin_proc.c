...
void *proc_main(void *ptr)
{
	...
	int vdo_tegra				= !config_get_boolean("plugin:proc", "tegra", 1);
	...
	unsigned long long sutime_sys_tegra = 0ULL;

	// the next time we will run - aligned properly
	unsigned long long sunext = (time_s() - (time_s() % rrd_update_every) + rrd_update_every) * 1000000ULL;
	...
	for(;1;) {
		...
		while((sunow = time_us()) < sunext)
			usleep((useconds_t)(sunext - sunow));

		// find the next time we need to run
		while(time_us() > sunext)
		...
		if(!vdo_tegra) {
			debug(D_PROCNETDEV_LOOP, "PROCNETDEV: calling do_tegra().");

			sunow = time_us();
			vdo_tegra = do_sys_tegra(rrd_update_every, (sutime_sys_tegra)?sunow - sutime_sys_tegra:0ULL);
			sutime_sys_tegra = sunow;
		}
		if(unlikely(netdata_exit)) break;
