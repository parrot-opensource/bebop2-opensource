...
procfile *procfile_readall(procfile *ff) {
	...
	if (ff->flags & PROCFILE_FLAG_NO_SEEK) {
		// seek not supported (typical on debugfs), just reopen the file for next read
		debug(D_PROCFILE, "Reopen file '%s' that does not support lseek", ff->filename);
		close(ff->fd);
		int fd = open(ff->filename, O_RDONLY, 0666);
		if(unlikely(fd == -1)) {
			if(unlikely(!(ff->flags & PROCFILE_FLAG_NO_ERROR_ON_FILE_IO))) error(PF_PREFIX ": Cannot open file '%s'", ff->filename);
			procfile_close(ff);
			return NULL;
		}
		ff->fd = fd;
