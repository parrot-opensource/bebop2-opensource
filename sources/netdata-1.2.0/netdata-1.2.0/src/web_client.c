...
void web_client_reset(struct web_client *w)
{
	...
	if(likely(w->last_url[0]))
		log_access("%llu: (sent/all = %ld/%ld bytes %0.0f%%, prep/sent/total = %0.2f/%0.2f/%0.2f ms) %s: %d '%s'",
			...
			(float)usecdiff(&w->ts_ready, &w->ts_in) / 1000.0,
			(float)usecdiff(&ts, &w->ts_ready) / 1000.0,
			(float)usecdiff(&ts, &w->ts_in) / 1000.0,
...
int web_client_api_request_v1_registry(struct web_client *w, char *url)
{
	...
	switch(action) {
		case 'A':
			return registry_request_access_json(w, person_guid, machine_guid, machine_url, url_name, time_s());

		case 'D':
			return registry_request_delete_json(w, person_guid, machine_guid, machine_url, delete_url, time_s());

		case 'S':
			return registry_request_search_json(w, person_guid, machine_guid, machine_url, search_machine_guid, time_s());

		case 'W':
			return registry_request_switch_json(w, person_guid, machine_guid, machine_url, to_person_guid, time_s());
...
void web_client_process(struct web_client *w) {
	...
	if(what_to_do < 0) {
		...
		clock_gettime(CLOCK_MONOTONIC, &w->ts_in);
	...
	clock_gettime(CLOCK_MONOTONIC, &w->ts_ready);
	w->response.data->date = time_s();
	w->response.sent = 0;
