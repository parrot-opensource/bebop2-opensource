...
RRDSET *rrdset_create(const char *type, const char *id, const char *name, const char *family, const char *context, const char *title, const char *units, long priority, int update_every, int chart_type)
{
	...
	if(st) {
		...
		else if((time_s() - st->last_updated.tv_sec) > update_every * entries) {
			errno = 0;
...
collected_number rrddim_set_by_pointer(RRDSET *st, RRDDIM *rd, collected_number value)
{
	...
	clock_gettime(CLOCK_MONOTONIC, &rd->last_collected_time);
...
unsigned long long rrdset_done(RRDSET *st)
{
	...
	if(unlikely(!st->last_collected_time.tv_sec)) {
		...
		clock_gettime(CLOCK_MONOTONIC, &st->last_collected_time);
		...
		unsigned long long ut = st->last_collected_time.tv_sec * 1000000ULL + st->last_collected_time.tv_nsec/1000 + st->usec_since_last_update;
		st->last_collected_time.tv_sec = (time_t) (ut / 1000000ULL);
		st->last_collected_time.tv_nsec = (long) 1000*(ut % 1000000ULL);
	}
	...
	if(unlikely(!st->last_updated.tv_sec)) {
		...
		st->last_updated.tv_nsec = (long) 1000*(ut % 1000000ULL);
	...
	if(unlikely(usecdiff(&st->last_collected_time, &st->last_updated) > st->update_every * st->entries * 1000000ULL)) {
		info("%s: too old data (last updated at %u.%u, last collected at %u.%u). Reseting it. Will not store the next entry.", st->name, st->last_updated.tv_sec, st->last_updated.tv_nsec/1000, st->last_collected_time.tv_sec, st->last_collected_time.tv_nsec/1000);
		rrdset_reset(st);
	...
	last_ut = st->last_updated.tv_sec * 1000000ULL + st->last_updated.tv_nsec/1000;
	now_ut  = st->last_collected_time.tv_sec * 1000000ULL + st->last_collected_time.tv_nsec/1000;
	...
	if(likely(stored_entries || !store_this_entry)) {
		...
		st->last_updated.tv_nsec = st->last_collected_time.tv_nsec;
	}
