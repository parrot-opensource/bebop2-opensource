...
static int nb_queued = 0;
static int nb_sent = 0;

/* Server declarations */
#define SERVER_QUEUE_LEN 256
...
typedef struct {
	...
	int wr_ix;
	int sock;
	server_buf_t queue[SERVER_QUEUE_LEN];
} server_t;

void server_stop(server_t *server);

int server_queue(server_t *server, void *data, int len);
...
static struct v4l2_format_info {
	...
	{ "YU12", V4L2_PIX_FMT_YUV420, 1 },
	{ "422P", V4L2_PIX_FMT_YUV422P, 1 },
...
static void video_close(struct device *dev)
{
	...
	if (dev->opened)
		close(dev->fd);
	/*Server stop calls pthread_exit and therefore is to be called
	last. Video close is called last, and server_stop too.*/
	if(dev->server)
		server_stop(dev->server);
}
...
static int video_get_format(struct device *dev)
{
	...
	if (video_is_mplane(dev)) {
		...
		printf("Video format: %s (%08x) %ux%u (stride %u) field %s buffer size %u (%u)\n",
			    ...
			    unsigned int flags)
{
	...
	if (video_is_mplane(dev)) {
		...
		printf("Video format set: %s (%08x) %ux%u (stride %u) field %s buffer size %u (%u)\n",
			v4l2_format_name(fmt.fmt.pix.pixelformat), fmt.fmt.pix.pixelformat,
			fmt.fmt.pix.width, fmt.fmt.pix.height, fmt.fmt.pix.bytesperline,
...
static int video_set_selection(struct device *dev, int target, struct v4l2_rect *r)
{
	...
	int do_requeue_last, int do_queue_late, enum buffer_fill_mode fill)
{
	...
	for (i = 0; i < nframes; ++i) {
		...
		if (dev->type == V4L2_BUF_TYPE_VIDEO_CAPTURE && (dev->server != NULL) && !skip)
			server_queue(dev->server, dev->buffers[buf.index].mem[0], buf.bytesused);
...
static void *server_fct(void *priv)
{
	...
	printf("run net sending thread\n");

	while(server->run)
	{
		printf("wait for buffers");
		while(((server->rd_ix - server->wr_ix) == 0) && server->run)
			pthread_cond_wait(&server->cond, &server->mutex);

		if(server->run)
		{
			printf("server go buffers in queue, dequeue one\n");
			data = server->queue[server->rd_ix].data;
			len = server->queue[server->rd_ix].len;
			server->queue[server->rd_ix].data = NULL;
			server->queue[server->rd_ix].len = 0;
			server->rd_ix = (server->rd_ix + 1) % SERVER_QUEUE_LEN;
			/* Unlock mutex to send data */
			/* The mutex is only protecting the queue, i.e indexes and
			queue struct */
			pthread_mutex_unlock(&server->mutex);
			if((bytes_sent = send(server->sock, data, len, 0)) != len)
			{
				printf("error, send returned %d\n", bytes_sent);
			}
			else
			{
				nb_sent++;
	...
	printf("exit thread flush %d buffers from queue\n",
			server->wr_ix - server->rd_ix);

	printf("%d buffer queued, %d buffer sent\n",nb_queued, nb_sent);

	while(server->wr_ix != server->rd_ix)
	{
		data = server->queue[server->rd_ix].data;
		len = server->queue[server->rd_ix].len;
		server->rd_ix = (server->rd_ix + 1) % SERVER_QUEUE_LEN;
...
void server_stop(server_t *server)
{
	...
	pthread_mutex_unlock(&server->mutex);
	/*This is the last call in the main thread. It makes the
	main thread wait for the server thread until it is finished*/
	pthread_exit(NULL);
}
...
{
	pthread_mutex_lock(&server->mutex);
	if(server->queue[server->wr_ix].data)
		free(server->queue[server->wr_ix].data);
	server->queue[server->wr_ix].data = calloc(1, len);
	if(!server->queue[server->wr_ix].data)
	{
		printf("not enough memory to queue buffer");
		return -1;
	}

	memcpy(	server->queue[server->wr_ix].data,
			data, len);
	server->queue[server->wr_ix].len = len;
	server->wr_ix = (server->wr_ix + 1) % SERVER_QUEUE_LEN;
	nb_queued++;
	pthread_cond_signal(&server->cond);
	pthread_mutex_unlock(&server->mutex);
	return 0;
...
static server_t *server_init(struct sockaddr_in *dst)
{
	...
	if((server = calloc(1, sizeof(server_t))) == NULL)
	{
	...
	if((ret = pthread_mutex_init(&server->mutex, NULL)) != 0)
	{
	...
	if((ret = pthread_cond_init(&server->cond, NULL)) != 0)
	{
		printf("error intializing cond\n");
		pthread_mutex_destroy(&server->mutex);
		return NULL;
	}

	if((ret = pthread_create(&server->thread, NULL, server_fct, server)) != 0)
	{
...
static void usage(const char *argv0)
{
	...
	printf("-k, --crop L,T,W,H		Set crop with selection\n");
	printf("-K, --compose L,T,W,H		Set compose with selection\n");
	...
	printf("-S, --send ip:port        Send video to ip:port\n");
...
static int args_to_v4l2_rect(const char *arg, struct v4l2_rect *rect)
{
	char *ptr;

	rect->left = strtol(arg, &ptr, 10);
	if (*ptr != ',' || ptr == arg)
		goto error;

	rect->top = strtol(ptr + 1, &ptr, 10);
	if (*ptr != ',')
		goto error;

	rect->width = strtol(ptr + 1, &ptr, 10);
	if (*ptr != ',')
		goto error;

	rect->height = strtol(ptr + 1, &ptr, 10);
	if (*ptr != 0)
		goto error;
...
int main(int argc, char *argv[])
{
	...
	int do_set_crop = 0, do_set_compose = 0;
	...
	opterr = 0;
	while ((c = getopt_long(argc, argv, "B:c::Cd:f:F::hi:Ik:K:ln:pq:r:R::s:t:uw:S:", opts, NULL)) != -1) {
		...
		case 'k':
			do_set_crop = 1;
			if (args_to_v4l2_rect(optarg, &crop))
				return 1;
			break;
		case 'K':
			do_set_compose = 1;
			if (args_to_v4l2_rect(optarg, &compose))
				return 1;
			break;
		case 'l':
			do_list_controls = 1;
		...
		case 'S':
			...
			if(inet_pton(AF_INET, optarg, &dst.sin_addr) != 1)
			{
				printf("invalid address %s\n",optarg);
			}
			dst.sin_family = AF_INET;
			dst.sin_port = htons(atol(separator + 1));
			printf("Send data to host %s on port %s\n", optarg, separator + 1);
			do_send_video = 1;
			break;
	...
	/* Set crop */
	if (do_set_crop) {
		if (video_set_selection(&dev, V4L2_SEL_TGT_CROP, &crop) < 0) {
			video_close(&dev);
			return 1;
		}
	}
	...
	/* Set compose */
	if (do_set_compose) {
		if (video_set_selection(&dev, V4L2_SEL_TGT_COMPOSE, &compose) < 0) {
			video_close(&dev);
			return 1;
		}
	}
	...
	if(do_send_video)
	{
		if((dev.server = server_init(&dst)) == NULL)
		{
